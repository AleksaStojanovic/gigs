    ...
    if form.validate_on_submit():
        ...
        if user.check_password(password):
            ...
            if form.remember_me.data:
                ...
                resp.set_cookie(
                    "user_id", encrypt_cookie(user.id), max_age=60 * 60 * 24 * 100
                )
                return resp
            return redirect(url_for("main.home"))
        else:
...
def activate_account(token):
    ...
    if current_user.activate(token):
        db.session.commit()
        flash(
            "Your account is confirmed. Welcome " + current_user.username + "!",
            "success",
        )
    else:
...
def update_password(token, email):
    ...
    flash(
        "Hi " + user.username + "! You can now set a new password for the account.",
        "success",
    )
